...
> **Rework pending — ADR 0024** supersedes ADR 0022's primary `shipping_overcharge` mechanism. Reading the real per-order exports showed each Platform discloses its own actual shipping, so detection becomes **per-platform + subsidy-aware** (TikTok billed-weight vs parcel-weight · Shopee income-report net the seller bore · Lazada per-cycle Shipping Fee statement), catalogue-compute demoted to fallback; plus manual Claim creation, native status/deadline/payout mirroring, and two new importers (Shopee income report, Lazada Shipping Fee statement). Not yet built — to be decomposed into Issues.
