...
**In an order, a Bundle arrives as one bundle Platform SKU** — no marketplace decomposes a kit into components in its order export (verified Shopee/Lazada/TikTok; ADR 0023). It resolves to the Bundle Variant and **decomposition is ours, from the BOM**: the **Pick List** shows the components (what you physically grab), the **Dispatch Manifest** shows the bundle (what the buyer ordered, with a toggle to components). The Platform "bundle deal" (Shopee) / `bundleId` (Lazada) columns are **promotions** (buy-together discount across real SKUs), **not** a Bundle/Kit — never conflate. **TikTok "Virtual Bundle"** is the same virtual-kit idea and likewise arrives as the bundle SKU (we explode); a tenant who lists with TikTok's native Virtual Bundle is a **known gap** — fail-loud until a real export sample is captured (ADR 0005).
_Also_: เซ็ต, ชุดสินค้า, kit, BOM
_Avoid_: Variant (a Bundle is a *kind* of Variant but holds no stock — its components do), Product (the catalog parent, not a sellable set), bundle deal / `bundleId` (a Platform **promotion**, not a Kit)
