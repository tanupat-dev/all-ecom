...
- **`docs/ROADMAP.md`** — build order (dependency-first, no-rework). Currently **Phases 0–5 complete**, **Phase 9 (Channel Upload, ADR 0019) in progress** — master fields, Product Image (R2-shaped disk), 3-platform existing-listing importers, Listing Coverage + Status, catalogue round-trip, POS image all landed (Issues #46–#56, #60). Remaining: the 3 template-fill slices **#57–#59, blocked on #50 (HITL — Tan provides real template files in `ref doc/`)** + ops #48 (R2 bucket). **Phase 6 Accounting** is planned but has no Issues yet (decompose with `all-ecom-to-issues` first).
