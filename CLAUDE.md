...
- **`docs/ROADMAP.md`** — build order (dependency-first, no-rework). Currently **Phases 0–6 + 9 complete**. Phase 6 (Accounting, ADR 0007 + **ADR 0020**: cycle-aware Accounting Entry with income+fee lines, 3-platform accounting importers, Platform Fee Profile + Expected Net, Reconciliation Status + Mismatch list, Expected Payout + overdue, Hold Period auto-tune, Expense, POS direct P&L, daily P&L rollup, combined P&L report; Issues #61–#72) shipped. Phase 9 (Channel Upload, ADR 0019; Issues #46–#60) shipped; only ops #48 (R2 bucket, prod-only) remains. Next = **Phase 7 Promotions + Margin** — decomposed into Issues #73–#77 (all `ready-for-agent`; ADR 0021 pins the Promotion-Line-authoritative Deal Price), build in progress.
