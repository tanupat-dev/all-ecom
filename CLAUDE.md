...
- **`docs/ROADMAP.md`** — build order (dependency-first, no-rework). Currently **Phases 0–2 complete** (foundations · catalog/stock kernel · identity/shop/order kernel); next = **Phase 3 POS slice** — the first shippable product (no Issues filed yet — decompose with `all-ecom-to-issues` first).
